...
{
	{
		4, 2, 2, 0, 2, 2, 1, 1, 0, qtrue	// JDC: this was tq 3
	},
	{
		3, 2, 2, 0, 0, 0, 1, 0, 0, qtrue
	},
	{
		2, 2, 1, 0, 1, 0, 0, 0, 0, qtrue
	},
	{
		2, 2, 1, 1, 1, 0, 0, 0, 0, qtrue
	},
	{
		3, 2, 1, 0, 0, 0, 1, 0, 0, qtrue
	}
};
...
static void GraphicsOptions_GetInitialVideo( void )
{
	...
	s_ivo.windowmode  = s_graphicsoptions.fs.curvalue;
...
static void GraphicsOptions_CheckConfig( void )
{
	...
	for ( i = 0; i < NUM_IVO_TEMPLATES; i++ )
	{
		...
		if ( s_ivo_templates[i].mode != s_graphicsoptions.mode.curvalue )
			continue;
		if ( s_ivo_templates[i].windowmode != s_graphicsoptions.fs.curvalue )
			continue;
...
static void GraphicsOptions_UpdateMenuItems( void )
{
	...
	{
		s_graphicsoptions.fs.curvalue = 2;
	...
	if ( s_graphicsoptions.fs.curvalue != 2 || s_graphicsoptions.driver.curvalue == 1 )
	{
	...
	if ( s_ivo.windowmode != s_graphicsoptions.fs.curvalue )
	{
		s_graphicsoptions.apply.generic.flags &= ~(QMF_HIDDEN|QMF_INACTIVE);
	}
...
static void GraphicsOptions_ApplyChanges( void *unused, int notification )
{
	...
	if (notification != QM_ACTIVATED)
		return;
	mode = s_graphicsoptions.mode.curvalue == MODE_DESKTOP ? -2 : s_graphicsoptions.mode.curvalue;
	...
	trap_Cvar_SetValue( "r_modeFullscreen", mode );
	trap_Cvar_SetValue( "r_fullscreen", s_graphicsoptions.fs.curvalue == 2 );
	trap_Cvar_SetValue( "r_noborder", s_graphicsoptions.fs.curvalue == 1 );
...
static void GraphicsOptions_Event( void* ptr, int event ) {
	...
	case ID_LIST:
		...
		s_graphicsoptions.fs.curvalue          = ivo->windowmode;
		break;
...
{
	if ( trap_Cvar_VariableValue( "r_fullscreen" ) && *UI_Cvar_VariableString( "r_modeFullscreen" ) )
	{
		s_graphicsoptions.mode.curvalue = atoi( UI_Cvar_VariableString( "r_modeFullscreen" ) );
	}
	else
	{
	...
	if ( s_graphicsoptions.mode.curvalue == -2 )
	{
		s_graphicsoptions.mode.curvalue = MODE_DESKTOP;
	}
	else if ( s_graphicsoptions.mode.curvalue < 0 )
	{
		s_graphicsoptions.mode.curvalue = 3;
	}
	s_graphicsoptions.fs.curvalue = trap_Cvar_VariableValue( "r_fullscreen" ) ? 2 :
		( trap_Cvar_VariableValue( "r_noborder" ) ? 1 : 0 );
...
void GraphicsOptions_MenuInit( void )
{
	...
	// references/modifies "r_fullscreen" and "r_noborder"
	s_graphicsoptions.fs.generic.type     = MTYPE_SPINCONTROL;
	s_graphicsoptions.fs.generic.name	  = "Window Mode:";
	...
	s_graphicsoptions.fs.itemnames	      = windowmode_names;
	y += BIGCHAR_HEIGHT+2;
