...
qboolean trap_AP_GetState( apUIState_t *state ) {
	return (qboolean)syscall( UI_AP_GET_STATE, state, sizeof( *state ) );
}

qboolean trap_AP_Connect( const apConnectRequest_t *request ) {
	return (qboolean)syscall( UI_AP_CONNECT, request, sizeof( *request ) );
}

void trap_AP_Disconnect( void ) {
	syscall( UI_AP_DISCONNECT );
}

qboolean trap_AP_StartStage( int mapIndex ) {
	return (qboolean)syscall( UI_AP_START_STAGE, mapIndex );
}
